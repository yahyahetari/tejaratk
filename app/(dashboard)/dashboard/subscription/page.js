    ...
    <div className="space-y-8 animate-fade-in px-4 sm:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-600 to-brand-700 flex items-center justify-center shadow-lg shadow-brand-600/20">
        ...
        {hasActiveSubscription && (
          <Link href="#plans">
            <button className="px-6 py-3 bg-gradient-to-r from-brand-700 to-brand-600 text-white rounded-xl font-bold flex items-center gap-2 hover:shadow-xl hover:shadow-brand-700/20 transition-all hover:scale-105 active:scale-95">
            ...
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div className="flex items-center gap-5">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-500 p-[1px] shadow-lg shadow-emerald-500/10">
                ...
                <span className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-emerald-400 text-sm font-bold flex items-center gap-2 w-fit">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 text-right">
                ...
                <div className="p-6 bg-white/[0.03] border border-white/5 rounded-2xl hover:bg-white/[0.05] transition-all">
                  ...
                  <p className="text-2xl font-black text-white">
                    {getPlanById(subscription.planId?.toUpperCase() || subscription.planType?.toUpperCase())?.name || subscription.planName || subscription.planType || 'خطة PREMIUM'}
                  </p>
                </div>
                ...
                <div className="p-6 bg-white/[0.03] border border-white/5 rounded-2xl hover:bg-white/[0.05] transition-all">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-gray-500 font-bold text-sm">/ {subscription.billingCycle === 'YEARLY' ? 'سنوي' : 'شهري'}</span>
                  </div>
                </div>
              ...
              {subscription.cancelAtPeriodEnd && (
                <div className="mb-8 p-5 bg-red-500/5 border border-red-500/10 rounded-2xl flex items-center gap-4">
                  ...
                  <Link href="/dashboard/subscription/resume">
                    <button className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-all shadow-lg shadow-emerald-500/20 text-sm">
                      استئناف
                    </button>
                  </Link>
                </div>
              ...
              <div className="pt-8 border-t border-white/5 flex flex-wrap items-center justify-between gap-6">
                <div className="flex items-center gap-3 opacity-60">
                  ...
                  <span className="text-gray-400 text-sm font-medium italic">دفع آمن 100% مدعوم بواسطة Paddle</span>
                </div>
                
                <div className="flex items-center gap-6">
                  {!subscription.cancelAtPeriodEnd && (
                    <Link href="/dashboard/subscription/cancel">
                      <button className="text-gray-500 hover:text-red-400 font-bold transition-colors text-sm underline underline-offset-8 decoration-white/10">
                        إلغاء الاشتراك
                      </button>
                    </Link>
                  )}
                  
                  <button className="px-6 py-3 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-xl font-bold text-sm transition-all shadow-xl flex items-center gap-2 group/btn">
                    إدارة وسيلة الدفع
                    <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform opacity-60" />
                  </button>
                </div>
              </div>
            </div>
      ...
      {!hasActiveSubscription ? (
        ...
        <div id="plans" className="scroll-mt-8 bg-gray-950 rounded-[2.5rem] mt-16 pt-12 border border-gray-800 shadow-2xl shadow-brand-500/10 overflow-hidden relative">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_hsla(38,65%,50%,0.05)_0%,transparent_50%)] pointer-events-none"></div>
          <div className="text-center mb-6 relative z-10">
            ...
            <p className="text-gray-400 mt-2 max-w-md mx-auto">هل تحتاج إلى مميزات أقوى؟ اختر الباقة الأنسب لتوسيع تجارتك.</p>
          </div>
          <Pricing
            showHeader={false} 
            ctaLink="/dashboard/subscription/checkout"
      ...
      {/* Invoices Card */}
      <div className="bg-[#12121a] border border-white/5 rounded-[2.5rem] p-8 mt-12 mb-12 animate-fade-in-up delay-400 shadow-2xl">
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center border border-white/10">
            <DollarSign className="h-6 w-6 text-gray-400" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white">سجل الفواتير</h2>
            <p className="text-gray-400">تاريخ عمليات الدفع والتحميل</p>
          </div>
        </div>
        ...
        <div className="text-center py-16 bg-white/[0.02] border border-dashed border-white/10 rounded-[2rem]">
          ...
          <p className="text-gray-300 font-bold mb-2">لا توجد فواتير حتى الآن</p>
          <p className="text-sm text-gray-500 max-w-xs mx-auto">بمجرد بدء اشتراكك، ستظهر جميع فواتيرك هنا للتحميل والمراجعة.</p>
        </div>
      </div>
    </div>
  );
