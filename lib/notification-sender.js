...
/**
 * إشعار إنشاء كود التفعيل
 * @param {string} merchantId - معرف التاجر
 * @param {string} key - كود التفعيل
 */
export async function notifyActivationKeyCreated(merchantId, key) {
  return await sendNotification(merchantId, {
    type: 'KEY',
    title: '🔑 تم إنشاء كود التفعيل',
    message: 'تم إنشاء كود التفعيل الخاص بك بنجاح. يمكنك الآن استخدامه في متجرك.',
...
/**
 * إشعار إعادة توليد كود التفعيل
 * @param {string} merchantId - معرف التاجر
 * @param {string} newKey - الكود الجديد
 */
export async function notifyActivationKeyRegenerated(merchantId, newKey) {
  return await sendNotification(merchantId, {
    type: 'KEY',
    title: '🔄 تم إعادة توليد كود التفعيل',
    message: 'تم إنشاء كود تفعيل جديد. يرجى تحديث الكود في متجرك.',
    link: '/dashboard/activation-key',
    data: { newKey }
  });
}
...
/**
 * إشعار استخدام كود التفعيل
 * @param {string} merchantId - معرف التاجر
 * @param {object} usageData - بيانات الاستخدام
 */
export async function notifyActivationKeyUsed(merchantId, usageData) {
  return await sendNotification(merchantId, {
    type: 'KEY',
    title: '✓ تم استخدام كود التفعيل',
    message: `تم استخدام كود التفعيل من ${usageData.ipAddress || 'موقع غير معروف'}`,
    link: '/dashboard/activation-key/usage',
    data: usageData
  });
}

/**
 * إشعار محاولة استخدام كود غير صالح
 * @param {string} merchantId - معرف التاجر
 * @param {object} attemptData - بيانات المحاولة
 */
export async function notifyInvalidKeyAttempt(merchantId, attemptData) {
  return await sendNotification(merchantId, {
    type: 'KEY',
    title: '⚠️ محاولة استخدام كود غير صالح',
    message: 'تم رصد محاولة استخدام كود تفعيل غير صالح. يرجى التحقق من أمان حسابك.',
    link: '/dashboard/activation-key',
    data: attemptData
  });
}
...
export async function notifyStoreSetupCompleted(merchantId) {
  return await sendNotification(merchantId, {
    ...
    message: 'تهانينا! تم إكمال إعداد متجرك بنجاح. يمكنك الآن البدء في استخدام كود التفعيل.',
    link: '/dashboard/activation-key'
  });
}
