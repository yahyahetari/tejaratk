...
export const PADDLE_PRODUCTS = {
  BASIC: {
    monthly: process.env.NEXT_PUBLIC_PADDLE_BASIC_MONTHLY_PRICE_ID || 'pri_basic_monthly',
    yearly: process.env.NEXT_PUBLIC_PADDLE_BASIC_YEARLY_PRICE_ID || 'pri_basic_yearly',
  },
  PREMIUM: {
    monthly: process.env.NEXT_PUBLIC_PADDLE_PREMIUM_MONTHLY_PRICE_ID || 'pri_premium_monthly',
    yearly: process.env.NEXT_PUBLIC_PADDLE_PREMIUM_YEARLY_PRICE_ID || 'pri_premium_yearly',
  },
  ENTERPRISE: {
    monthly: process.env.NEXT_PUBLIC_PADDLE_ENTERPRISE_MONTHLY_PRICE_ID || 'pri_enterprise_monthly',
    yearly: process.env.NEXT_PUBLIC_PADDLE_ENTERPRISE_YEARLY_PRICE_ID || 'pri_enterprise_yearly',
  },
};
...
export function verifyPaddleWebhook(rawBody, signature) {
  ...
  if (!webhookSecret || !signature) {
    console.error('Missing PADDLE_WEBHOOK_SECRET or signature is empty');
    return false;
  }

  try {
    // Parse Paddle v2 signature header (Example: "ts=1690000000;h1=xxxxxxxxxxxxx")
    ...
    // Paddle v2 payload representation is "ts:rawBody"
...
async function handleSubscriptionCreated(data) {
  const { id, customer_id, items, status, current_billing_period, custom_data } = data;
  ...
  return {
    ...
    data: {
      merchantId: custom_data?.merchantId,
      userId: custom_data?.userId,
      paddleSubscriptionId: id,
      paddleCustomerId: customer_id,
      planId: custom_data?.planId || planType,
      planName: custom_data?.planId || planType,
      planType,
      billingCycle: custom_data?.billingCycle || billingCycle,
      status,
...
async function handleSubscriptionUpdated(data) {
  const { id, status, current_billing_period, scheduled_change, custom_data } = data;
...
async function handleTransactionCompleted(data) {
  const { id, subscription_id, customer_id, details, custom_data } = data;
  ...
  return {
    ...
    data: {
      ...
      status: 'ACTIVE', // A completed transaction means an active subscription phase
      planId: custom_data?.planId,
      planName: custom_data?.planId,
      planType: getPlanTypeFromPriceId(details?.line_items?.[0]?.price?.id),
      billingCycle: custom_data?.billingCycle,
    },
  };
}
...
function getPlanTypeFromPriceId(priceId) {
  ...
  // Compare against actual Paddle Price IDs from environment variables
  const enterprisePrices = [
    process.env.NEXT_PUBLIC_PADDLE_ENTERPRISE_MONTHLY_PRICE_ID,
    process.env.NEXT_PUBLIC_PADDLE_ENTERPRISE_YEARLY_PRICE_ID,
  ];
  const premiumPrices = [
