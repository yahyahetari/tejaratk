          ...
          return (
            <Link key={action.href} href={action.href}>
              <div className={`bg-white/[0.03] p-3 sm:p-4 lg:p-5 group hover:scale-[1.03] transition-all duration-300 relative h-full rounded-lg sm:rounded-xl lg:rounded-2xl border border-white/5 ${action.hoverBg} ${action.featured
                ...
                <div className={`w-10 h-10 sm:w-11 sm:h-11 lg:w-12 lg:h-12 xl:w-14 xl:h-14 rounded-lg sm:rounded-xl lg:rounded-2xl bg-gradient-to-br ${action.gradient} flex items-center justify-center mb-2 sm:mb-3 lg:mb-4 shadow-lg ${action.shadow} group-hover:scale-110 transition-transform`}>
                  <Icon className="h-5 w-5 sm:h-5.5 sm:w-5.5 lg:h-6 lg:w-6 xl:h-7 xl:w-7 text-white" />
                </div>

                <h3 className="text-xs sm:text-sm lg:text-base xl:text-lg font-bold text-gray-200 mb-1 sm:mb-2 group-hover:text-white transition-colors line-clamp-1">
                  {action.title}
                </h3>
